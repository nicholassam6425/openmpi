...
//functions for long long to mpz_t and vice versa
...
{
    // unsigned long long MAX_NUMBER = 10000000; // ten million
    // unsigned long MAX_NUMBER = 1 000 000 000; //one billion
    unsigned long long MAX_NUMBER = 1000000000000; //one trillion
    ...
    unsigned long long message[5];
    unsigned long long my_start, n, my_prime_start, my_largest_prime_gap, my_prev_prime, largest_prime_gap, left_prime, right_prime; // to indicate the split per processor
    MPI_Status status;
    unsigned long long *final_array;
    ...
    n = (unsigned long long)(MAX_NUMBER / num_procs);
    my_start = n * my_rank;
    ...
    mpz_set_ull(my_cursor, my_start); // this is equivalent to my_cursor = my_start
    while (mpz_probab_prime_p(my_cursor, NUM_REPS) == 0)
    {
        mpz_set_ull(my_cursor, mpz_get_ull(my_cursor)+1); // this is equivalent to my_cursor++
    }
    my_prime_start = mpz_get_ull(my_cursor); // this is equivalent to my_prime_start = my_cursor
    ...
    my_largest_prime_gap = mpz_get_ull(my_cursor) - my_prev_prime;
    message[3] = my_prev_prime;
    message[4] = mpz_get_ull(my_cursor);

    // find largest prime gap and related information in split
    while (mpz_get_ull(my_cursor) < my_start + n)
    { // this is equivalent to my_cursor <= my_start +n
        my_prev_prime = mpz_get_ull(my_cursor);
        mpz_nextprime(my_cursor, my_cursor); // set my_cursor to the next prime
        if (mpz_get_ull(my_cursor) - my_prev_prime > my_largest_prime_gap)
        {
            my_largest_prime_gap = mpz_get_ull(my_cursor) - my_prev_prime;
            message[3] = my_prev_prime;
            message[4] = mpz_get_ull(my_cursor);
    ...
    printf("DEBUG: Process %d found largest prime gap %lli, between %lli and %lli, in split between %lli and %lli\n", my_rank, message[2], message[3], message[4], my_start, my_start + n);
    if (my_rank == 0)
    { // master proc
        int i;   
        // append all the messages to final array, starting with master proc's message
        final_array = malloc(5 * num_procs * sizeof(unsigned long long));
        memcpy(final_array, message, 5 * sizeof(unsigned long long));
        ...
        for (source = 1; source < num_procs; source++)
        {
            ...
            MPI_Recv(message, sizeof(5 * sizeof(unsigned long long)), MPI_UNSIGNED_LONG, source, tag, MPI_COMM_WORLD, &status);
            printf("\nDEBUG: Message from processor %d: {%lli, %lli, %lli, %lli, %lli}", source, message[0], message[1], message[2], message[3], message[4]);
            memcpy(final_array + (5*source), message, 5 * sizeof(unsigned long long));
        }
        ...
        unsigned long long a, b; //temp vars for processing
        ...
        printf("\nThe largest gap between consecutive prime numbers under %lli is %lli, between %lli and %lli.\nTime elapsed:%f\n", MAX_NUMBER, largest_prime_gap, left_prime, right_prime, time_elapsed);
    }
    else
    { // slave proc
        MPI_Send(message, sizeof(5 * sizeof(unsigned long long)), MPI_UNSIGNED_LONG, dest, tag, MPI_COMM_WORLD);
    }
    end_time = MPI_Wtime();
    time_elapsed = end_time - start_time;
    MPI_Finalize();
