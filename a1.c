...
int main(int argc, char **argv) {
    ...
    while (mpz_probab_prime_p(my_cursor, NUM_REPS) == 0) {
        mpz_set_ui(my_cursor, mpz_get_ui(my_cursor)+1); //this is equivalent to my_cursor++
    }
    ...
    if (my_rank == 0) { //master proc
        ...
        for(i = 1; i < num_procs; i++) {
            ...
            MPI_Recv(message, sizeof(message), MPI_UNSIGNED_LONG, i, 0, MPI_COMM_WORLD, &status);
            memcpy(final_array + (5*i), message, 5 * sizeof(unsigned long));
        }
